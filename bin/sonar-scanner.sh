...
# Load the local dev secrets file if it exists. Kept outside the web root
# (not $PROJECT_ROOT) — this theme directory is served directly by Apache,
# and a .env here was previously publicly downloadable.
if [[ -f "$HOME/.env" ]]; then
  set +u
...
# Run JS unit tests and collect coverage
...
if [[ "$JS_EXIT" -ne 0 ]]; then
  echo ""
  echo "Error: tests failed (JS exit $JS_EXIT) — skipping SonarCloud scan."
  exit 1
fi
